...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    await target.send({
      components: buildModerationNoticeComponents({ action: 'ban', caseNum, reason }),
      flags: V2_FLAGS,
    ...
    try {
      ...
      return message.reply(`${EMOJI_TEXT.cross} Failed to ban user. I may be missing permissions.`);
    }
    ...
    return message.reply(`${EMOJI_TEXT.check} Banned ${target.user.tag} | Case #${caseNum}`);
  }
};
